...
public class HUDNavigationSystemEditor : HUDNavigationBaseEditor
{
	...
	protected override void OnBaseInspectorGUI ()
	{
		...
		if (_system_)
		{
			...
			EditorGUILayout.PropertyField(_pKeepAliveOnLoad, new GUIContent("Don't Destroy On LoadSceneCor", "If enabled, the system will automatically persist between scene changes. DontDestroyOnLoad()."));
			// CONTENT ENDOF
		}
		EditorGUILayout.EndVertical();
		...
		if (hudTarget.useRadar) {
			...
			if (_radar_) {
				...
				if (hudTarget.useRadarFading)
				{
					GUILayout.Space(4); // SPACE
					EditorGUILayout.BeginVertical();
					EditorGUILayout.Slider(_pRadarFadeDistance, 1f, 100f, "FadeInImage Distance");
		...
		if (hudTarget.useIndicators) {
			...
			if (_indicator_) {
				...
				if (hudTarget.useIndicatorFading) {
					GUILayout.Space (4); // SPACE
					EditorGUILayout.BeginVertical ();
					EditorGUILayout.Slider (_pIndicatorFadeRadius, 1f, 500f, "FadeInImage Radius");
			...
			if (_minimap_) {
				...
				if (hudTarget.minimapProfile != null) {
					...
					if (hudTarget.useMinimapFading)
					{
						GUILayout.Space(4); // SPACE
						EditorGUILayout.BeginVertical();
						EditorGUILayout.Slider(_pMinimapFadeDistance, 1f, 100f, "FadeInImage Distance");
